...
for i in 0..node['order_process']['num_instance']-1
	...
	version = node['order_process'][i]['version']
	...
	order_process_port = node['order_process'][i]['port']
	...
	remote_file "#{mod_dir}/#{package}" do
		source node['order_process'][i]['url']
		checksum node['order_process'][i]['checksum']
	  #mode "0755"
	end
	...
	template "#{ew_config_dir}/embedded-webapp.conf" do
		...
		notifies :restart, "service[order-process-#{i}]"
	end
	...
	# link folder
	link node['order_process'][i]['home'] do
		to app_dir
	end

	include_recipe "deamon-service::order-process"

	service "order-process-#{i}" do
