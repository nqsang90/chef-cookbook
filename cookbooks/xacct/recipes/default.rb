...
for i in 0..node['xacct']['num_instance']-1
	...
	remote_file "#{mod_dir}/#{package}" do
		source node['xacct'][i]['url']
		checksum node['xacct'][i]['checksum']
	end
	...
	template "#{ew_config_dir}/embedded-webapp.conf" do
		...
		variables(
			:service_port => node['xacct'][i]['port'],
			:production => false
		)
		notifies :restart, "service[xacct-#{i}]"
	end
